(function () {
  ...
  window.addEventListener('message', async (event) => {
    ...
    if (action === 'GET_STORAGE') {
      try {
        chrome.storage.local.get('ogx_lic_v2', (d) => {
          window.postMessage({ target: 'OGX_PAGE_SCRIPT', id, ok: true, result: d ? d.ogx_lic_v2 : null }, '*');
        });
      } catch (err) {
      ...
      try {
        chrome.storage.local.set({ ogx_lic_v2: payload }, () => {
  ...
  const s = document.createElement('script');
  s.src = chrome.runtime.getURL('gate.js');
